...
export function buildToolConfiguration(params: ToolConfigurationParams): ToolConfiguration {
  ...
  const config: ToolConfiguration = {
    ...
    initiate_login_uri: `${baseUrl}/${initPath.replace(/^\//, '')}`,
    redirect_uris: [`${baseUrl}/${redirectPath.replace(/^\//, '')}`],
    client_name: clientName,
    jwks_uri: `${baseUrl}/${jwksPath.replace(/^\//, '')}`,
    logo_uri: `${baseUrl}/${logoPath.replace(/^\//, '')}`,
    client_uri: baseUrl,
