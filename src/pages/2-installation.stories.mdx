...
Finally, there is an optional global stylesheet that sets some sensible defaults to make @atomicjolt/atomic-elements look its best, but
it shouldn't be necessary and you should be able to not include this if not desired

```js
import "@atomicjolt/atomic-elements/dist/global.css";
```
...
```js
import Button from "@atomicjolt/atomic-elements/components/Buttons/Button";
```
